...
import time
import senxor
from senxor.proc import apply_colormap, enlarge, normalize, get_colormaps
...
def main():
    ...
    try:
        ...
        time.sleep(0.1)
        logger.info(f"Successfully connected to SenXor device: {mi48.get_sn()}")

    except Exception as e:
        logger.critical(f"Cannot connect to SenXor: {e}")
        if mi48:
            mi48.close()
        sys.exit(1)
    ...
    stream_size = (640, 480)
    mi48.set_read_temp_units(temp_units='C')
    ...
    while True:
        ...
        cv.putText(img_enlarged_rgb, f"{max_val:.1f}C", (max_loc_scaled[0] + 10, max_loc_scaled[1]), CVFONT, CVFONT_SIZE, WHITE, 1)
